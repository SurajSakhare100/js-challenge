...
const fetchInvalidSpells = () => {
  return fetch("https://invalid-url.com").then((res) => res.json());
};
...
const fetchSpell = () =>
  fetch("https://potterapi-fedeperin.vercel.app/en/spells").then((res) =>
    res.json()
  );
const fetchHouses = () =>
  fetch("https://potterapi-fedeperin.vercel.app/en/houses").then((res) =>
    res.json()
  );
...
Promise.allSettled([promiseA, promiseB, promiseC]).then((results) => {
  results.forEach((result, index) => {
    if (result.status === "fulfilled") {
      console.log(
        `Task 10: Promise ${index + 1} fulfilled with value:`,
        result.value
      );
    } else {
      console.log(
        `Task 10: Promise ${index + 1} rejected with reason:`,
        result.reason
      );
    }
  });
